...
public class MyListAdapter extends ArrayAdapter<MyCostumRow> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		String str3 = data.get(position).getStatus();
        if(str3.equals("Take"))
            row.setBackgroundColor(Color.parseColor("#7e03ff91"));
        else
            row.setBackgroundColor(Color.parseColor("#7eff0091"));
        String str4 = data.get(position).getDate();
        holder.date.setText(str4.toString());
